...
## FV2 Bundle Path
fv2_bundle_path = "/Library/Security/SecurityAgentPlugins/FV2AuthPlugin.bundle"

## Mechs that support FV2AuthPlugin
fv2_mechs = ["FV2AuthPlugin:add-users"]
...
def edit_authdb():
    ...
    for p in [system_login_console_plist]:
        ...
        ## Remove FV2 mechs
        d = remove_mechs_in_db(d, fv2_mechs)
...
def remove_supporting_files():
    files_to_remove = []
    dirs_to_remove = [fv2_bundle_path]
...
def check_os():
    ...
    if OSVersion < 7:
        sys.exit("\nOnly OS X 10.7 and above can run this script\n")
